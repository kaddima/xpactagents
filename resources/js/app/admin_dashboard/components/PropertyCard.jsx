...
const PropertyCard = ({data :v,listState=false}) => {
  ...
  return (
        ...
        <div className="max-h-[400px] relative">
            ...
            {v.creator_id == currentUser.id && <div className='absolute top-10 left-2 px-1 bg-[#238f5d] rounded text-white' title='This property belongs to you'>
            ...
            <div className="px-3 ">
                <div className='border-b dark:border-b-slate-800 py-2 border-slate-300'>
            ...
            <div className='mt-auto'>
                <div className='flex justify-end space-x-2 md:space-x-4 items-start'>
                    <button onClick={()=>navigate(`/admin/listings/${v?.id}`)}  className='text-sm text-[#d92228] p-2 rounded-md hover:bg-slate-200'>View</button>
                   
                    <button onClick={()=>navigate(`/admin/listings/${v?.id}/edit`)} className='p-2 rounded-md hover:bg-slate-200'>
                        <FaEdit/>
                    </button>
                    
                    <button onClick={(e)=>{e.stopPropagation();onDelete(v.id,v.creator_id)}}  className='p-2 rounded-md hover:bg-slate-200'>
                        <FaTrash/>
                    </button>
