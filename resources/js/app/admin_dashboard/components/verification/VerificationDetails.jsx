...
const VerificationDetails = ({ details, setDetailsModal }) => {
	...
	const onResponse = (e) => {
		let type = e.target.dataset.type.toLowerCase()
		...
		if (window.confirm(`${type} this request?`)) {
			...
			showLoading()
			const url = `/admin/users/verification/${type}`
			Axios({
				url: url,
				method: "post",
				data: body
	...
	useEffect(() => {
		Axios.get(`/admin/users/${details.user_id}`).then(data => {
						...
						<div className='md:w-2/5'>
							<div className='md:pr-3 mt-4 space-y-5'>
								...
								{details.image && <div>
									...
									<img src={details.image} alt="" className='h-[22rem] w-full object-contain' />
