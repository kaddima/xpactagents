...
const TableRow = ({data,setUsersDetails})=>{
    ...
    return <tr>
        ...
        <td className='border-b text-sm dark:border-b-slate-800'>
            <span className={`text-sm text-white text-center block rounded px-1 
            ${data.is_agent == 1 && data.is_admin == 0 && 'bg-[#2b969e]'} 
            ${data.is_agent == 0 && 'bg-[#268a33]'} 
            ${data.is_agent == 1 && data.is_admin == 1 && 'bg-[#1d466d]'}`}>

                {data.is_agent == 1 && data.is_admin == 0 && 'Agent'} 
                {data.is_agent == 0 && 'User'} 
                {data.is_agent == 1 && data.is_admin == 1 && 'Admin'}
            </span>
        </td>
...
const AdmsUsers = () => {
    ...
    useEffect(()=>{
        ...
        Axios.get('/adms/users').then(data=>{
    ...
    <div className='h-full p-3'>
        <div>
            <div>
                ...
                <p className='text-xs font-semibold leading-none'>This is the full list of all users on the application</p>
            </div>  
            <div className='md:flex justify-between mt-5'>
                <div></div>
                <div className='w-full md:w-3/5'>
                    <SearchBox type={'all'} statefn={setUsersDetails}/>
                </div>
            </div>  
