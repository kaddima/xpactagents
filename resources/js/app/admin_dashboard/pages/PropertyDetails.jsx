...
import { BsCardImage, BsChevronDown, BsChevronUp } from "react-icons/bs"
...
const PropertyDetails = () => {
	...
	useEffect(() => {
		Axios.get(`/admin/properties/${propertyID}`).then(data => {
			...
			<div className='mt-5 md:w-10/12 mx-auto px-3 md:px-0'>
				<div className='md:flex justify-between items-center pb-5'>
					...
					<div className='md:flex items-center md:gap-3 space-y-3 md:space-y-0'>
						...
						<button onClick={() => onDelete(propertyDetails.id)}
							className={`border-[2px] block w-full md:w-auto text-xs px-5 transition py-2 
						rounded-md hover:bg-theme-color hover:text-white mt-3 md:mt-0`}>
							<div>
				...
				<div className='h-[16rem] md:h-[22rem] rounded-lg'>
					<div className='h-full flex md:flex-row flex-col space-x-1 relative'>
						...
						<button onClick={() => setShowPhotoManager(true)}
							className="absolute bottom-4 space-x-3 right-4 flex py-2 px-5 items-center bg-white
						 dark:bg-main-dark-bg  border border-black/80 rounded-md">
							<BsCardImage />
							<p className='font-semibold'>
								<span>{propertyDetails?.images && propertyDetails?.images.length}</span>
								Photos
							</p>
						</button>
					</div>
