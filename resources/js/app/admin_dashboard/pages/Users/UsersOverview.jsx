						...
						<div className='flex justify-between items-center w-full relative z-[1] bg-transparent'>
							<div className='text-white space-y-2'>
								...
								<p className='text-xs md:text-sm leading-none'>
									This represents the total number of users account registered on this application</p>
							</div>
							<div className='text-lg md:text-2xl text-white font-bold ml-2 md:pl-0'>
								{usersDetails?.totalUsersCount}
							</div>
						</div>
					</div>
					<div className='mt-5'>
						<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
							<Card name="Users - Regular" 
							number={usersDetails?.usersCount} 
							icon={<FaUserCheck size={32} className='text-sky-600' />} />

							<Card name="Users - Agent" 
							number={usersDetails?.agentsCount} 
							icon={<FaRegUserCircle size={32} className='text-green-600' />} />
							{/* <Card name="Properties for short let" number={7} icon={<MdApartment size={32} className='text-theme-color'/>}/> */}
						</div>
						<div className='mt-5'>
							<Card name="Users - Admin" 
							number={usersDetails?.adminsCount} 
							className={`py-5 text-xl`} 
							icon={<MdAdminPanelSettings size={32} className='text-orange-600' />} />
						</div>
					</div>
				</div>
					...
					<div className='h-full p-2 bg-neutral-100 dark:bg-slate-900'>
						...
						<div>
							<table className='w-full table-fixed'>
								...
								<tbody>
									{usersDetails?.latestUsers && usersDetails.latestUsers.map((v, i) => {
