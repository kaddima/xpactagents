...
const UserProfile = ({ activeUserInfo }) => {
	...
	const resolveMsg = () => {
		showLoading()
		Axios.post('/agents/properties/conversation/resolve', { conversation_id }).then(result => {
		...
		<div className='py-8'>
			<div className='mb-2 px-3'>
				...
				<p className='text-xs text-neutral-500 leading-none'>
					The informations below are for the current user you are chatting with.</p>
			</div>
			...
			<div className='flex text-sm mt-8 border-b dark:border-b-slate-800 pb-2'>
				<div className='w-2/4 text-center border-r dark:border-r-slate-800'>
					<a href={`mailto:${activeUserInfo?.email}`}
						className='bg-sky-200 h-[40px] w-[40px] rounded-full flex justify-center items-center mx-auto'>
						<MdEmail className='text-sky-800' size={20} />
					</a>
					<p>Email</p>
				</div>
				<div className='w-2/4 text-center'>
					<a href={`tel:${activeUserInfo?.phone}`}
						className='bg-sky-200 h-[40px] w-[40px] rounded-full flex justify-center items-center mx-auto'>
			...
			<div className='text-[13px] px-3 mt-3'>
				<button onClick={resolveMsg}
					className='bg-theme-color py-2 block w-full font-semibold text-white'>
					Close conversation
				</button>
			</div>
		</div>
