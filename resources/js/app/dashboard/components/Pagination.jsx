  ...
  return (
    <div className='w-full'>
        <div className='flex space-x-2'>
            <Link to={`/admins/property/category/${category}?page=${pageNo != 1 ? pageNo-1 : 1}`}
            ...
            {links.slice(1, links.length -1).map((v,i)=>{
                ...
                return <Link to={`/admins/property/category/${category}?page=${v.label}`}
                     className={`${v.active && `bg-green-700`} w-5 h-5 text-center bg-theme-color rounded text-white text-sm`} >
                    {v.label}
                </Link>
            
            })}
            <Link to={`/admins/property/category/${category}?page=${pageNo != totalPage ? +pageNo + 1 : totalPage}`}
                className="flex space-x-1 items-center bg-slate-600 text-white font-bold text-xs px-1 rounded">
                <span>Next</span> 
                <BsArrowRightShort/>
            </Link>
