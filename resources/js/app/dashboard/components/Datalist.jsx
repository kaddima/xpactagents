...
const Datalist = ({setValues,options,name}) => {
    ...
    const onAdd = (data)=>{
        ...
        setValues(state=>{
            ...
            for (let i = 0; i < state.length; i++) {
                ...
                if(state[i].amenities == data.amenities){
                    return state
                }
            }
  ...
  return (
    <div className='absolute left-0 bg-white border md:w-[35vw] max-w-[230px] text-slate-600 add-amenities'>
        <input {...register('amenities',{required:"Please provide a genre"})} id='input' list={'value'} className='bg-transparent form-input w-full ' placeholder='Enter genre'/>
        <datalist id={'value'}>
        ...
        <div className='flex absolute text-xs -top-3 right-0 gap-3'>
            <button onClick={handleSubmit(onAdd)} className='bg-[#65c0cf] text-white px-2 py-1 font-bold rounded'>Add</button>
            <button onClick={()=>{setShowForm(false);$('#input').val('')}} className=' bg-[#65c0cf] text-white font-bold px-2 py-1 rounded'>Cancel</button>
        </div>
    </div>
  )
}
