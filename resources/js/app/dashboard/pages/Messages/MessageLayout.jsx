					...
					<div className={`${showPOI ? 'translate-x-0' : 'translate-x-[-1000px]'} absolute  md:relative  md:translate-x-0 transition z-[200] bg-white dark:bg-slate-900 `}>
						...
						<div className='space-y-5 max-h-[60vh] overflow-y-scroll'>
							{propertyOfInterest.data.map((v, i) => {
								...
								return <div key={i} onClick={() => { setSearchParams({ 'property-of-interest': v.property_details.id }) }} className={`w-full px-2 text-[10px] relative transition cursor-pointer hover:bg-neutral-100 dark:hover:bg-slate-800 ${searchParams.get('property-of-interest') == v.property_details.id ? 'border-r-[2px] border-sky-600' : ''}`}>

									{(parseInt(v.unread_messages_count) > 0) &&
										<div className='notification' style={{ top: '2px', right: '6px' }}></div>
									}

									<h1 className='overflow-ellipsis overflow-hidden font-[400] whitespace-nowrap w-full uppercase'>{v.property_details.name}</h1>
									...
									<div className='flex justify-between items-center'>
										<p><BsEye size={13} onClick={(e) => { e.stopPropagation(); navigate(`/dashboard/property/${v?.id}`) }} className='cursor-pointer' /></p>
