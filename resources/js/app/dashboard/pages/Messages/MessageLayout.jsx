                    ...
                    <div className={`${showPOI ? 'translate-x-0':'translate-x-[-1000px]'} absolute  md:relative  md:translate-x-0 transition z-[200] bg-white dark:bg-slate-900 `}>
                        ...
                        <div className='space-y-5 max-h-[60vh] overflow-y-scroll'>
                            {propertyOfInterest.map((v,i)=>{
                                ...
                                return <div key={i} onClick={()=>{setSearchParams({'property-of-interest':v.property_id})}} className={`w-full px-2 text-[10px] relative transition cursor-pointer hover:bg-neutral-100 dark:hover:bg-slate-800 ${searchParams.get('property-of-interest') == v.property_id ? 'border-r-[2px] border-sky-600' : ''}`}>
                                        {msgNotifier.map((j,k)=>{
