...
const Question = ({ propertyDetails }) => {
	...
	const onSubmit = (formdata) => {
		...
		methods.reset()
		showLoading()
		Axios.post('/properties/conversations',
			{
				...formdata,
				property_id
			}).then((data) => {
				...
				<div className='absolute z-[10] inset-0 transition translate-x-[2000px] dark:bg-main-dark-bg bg-white opacity-90' id='q-submitted'>
					<div className='flex flex-col justify-center h-full w-full px-5'>
						<div>
							...
							<h1 className='text-lg font-semibold mt-3 mb-5 md:mb-1'>Message Submitted</h1>
					...
					<div className='text-sm'>
						<div className='flex items-center space-x-2'>
							<h1 className='font-semibold text-sky-800 text-lg'>
								{propertyDetails?.agentDetails.first_name} {propertyDetails?.agentDetails.last_name}
							</h1>
							{propertyDetails?.agentDetails.id_verified == 1 ?
								<MdVerified size={24} className='text-sky-600' />
								: null}
						</div>

						<p>{propertyDetails?.agentDetails.state} Xpactagent</p>
						<span>{propertyDetails?.agentDetails.first} typically replies in
							<span className='font-semibold text-sky-800'>few minutes</span>
						</span>
					</div>
				</div>
					...
					<div >
						<FormProvider {...methods}>
							<form onSubmit={methods.handleSubmit(onSubmit)} action="" className='relative'>
								...
								{showDetailsForm &&
									<UserDetails agentId={propertyDetails.creator_id}
								...
								<div className='border border-black dark:border-slate-800 min-h-[200px] pb-4'>
									<textarea name="" id=""
										...
										rows="6"
										className='form-textarea w-full border-0 dark:bg-transparent'
										placeholder='Write a message...' >

									</textarea>
									{!methods.watch('message')?.length && (<div className='flex flex-wrap mt-2 gap-5 px-5'>
										<button type='button' onClick={msgClick}
											className='border border-sky-800 rounded text-sky-800 font-semibold md:px-3 md:py-2'>
								...
								<div className='mt-4 flex items-center gap-5'>
									...
									<p className='text-lg'>Text or call
										<span className='text-sky-800 font-semibold pl-1'>{propertyDetails?.agentDetails.phone}</span>
									</p>
