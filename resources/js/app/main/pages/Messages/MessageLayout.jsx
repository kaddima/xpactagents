...
const MessageLayout = () => {
  ...
  <div className=' text-slate-800 w-full rounded-xl h-full max-w-[2520px] mx-auto xl:px-20 md:px-10 sm:px-2 px-4 pt-5'>
    <h1 className='text-2xl font-bold '>Messages</h1>
    <div className='flex flex-col md:flex-row space-x-1 md:h-[calc(100%-70px)] overflow-hidden mt-5'>
      
    <div className='bg-neutral-100 md:flex-shrink-0 md:w-[200px] pt-2 pb-2 md:h-full text-black'>
    <div className='block md:hidden'>
      <FaEllipsisV onClick={()=>setShowPOI(prev=>!prev)} className='float-right'/>
      <div className='clear-both'></div>
    </div>
    <div className={`${showPOI ? 'translate-x-0':'translate-x-[-1000px]'} absolute  md:relative  md:translate-x-0 transition z-[200] bg-white md:bg-transparent shadow-lg md:shadow-none`}>
      ...
      {propertyOfInterest?.map((v,i)=>{
        ...
        return <div key={i} onClick={()=>{setSearchParams({'property-of-interest':v.property_id,'conversation-id':v.conversation_id})}} className={`w-full px-2 text-[10px] transition cursor-pointer hover:bg-neutral-100 ${searchParams.get('property-of-interest') == v.property_id ? 'border-l-[2px] border-sky-600' : ''}`}>
